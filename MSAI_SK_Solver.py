...
class MSAI_SK_Solver():
    ...
    def null_report(self, df=None):
        ...
        plt.xticks(np.arange(-0.3, n_features - 0.3, 1), array.index,
                   rotation=45)
    ...
    def get_correlations(self, df=None, predict_column=None,
                         mess='correlations'):
        if df is None:
            df = self.X
        ...
        correlations_relatives = correlation[predict_column]
        correlations_relatives = correlations_relatives.sort_values(ascending=False)
        print("\n" + mess + ' relatives = ' + "\n",
        ...
        #         color="r", yerr=std[indices], align="center")
        plt.bar(range(n_features), correlations_relatives,
                color="b", align="center")
        plt.xticks(range(n_features), correlations_relatives.keys(),
                   rotation=45)
        plt.xlim([-1, n_features])
        plt.show()

        correlations_absolues = correlation[predict_column].abs()
        correlations_absolues = correlations_absolues.sort_values(ascending=False)
        ...
        # plt.bar(range(n_features), importances[indices],
        #         color="r", yerr=std[indices], align="center")
        plt.bar(range(n_features), correlations_absolues,
                color="b", align="center")
        plt.xticks(range(n_features), correlations_absolues.keys(),
                   rotation=45)
        plt.xlim([-1, n_features])
        plt.show()
    ...
    def pca_analysis(self, df=None, n_comp=20):
        ...
        for j in range(pca.n_components_):
            print('ratio', pca.explained_variance_ratio_[j],
                  'var', pca.explained_variance_[j],
                  'val', pca.singular_values_[j],
                  'mean', pca.mean_[j])
    ...
    #    print('Axis_2 uniques',type(Axis_2),"\n",Axis_2.unique())
        ...
        estimator.fit(data)
        xx, yy = np.meshgrid(np.linspace(Axis_1.min(), Axis_1.max(),
                                         len(Axis_1.unique())),
                             np.linspace(Axis_2.min(), Axis_2.max(),
                                         len(Axis_2.unique())))
    ...
    def plot_most_important(self, df=None, axis=None, corr=None,
                            nb=18, start=1):
        if df is None:
            df = self.X
        if axis is None:
    ...
    def feed_data(self, df, predict_column, n_first=None, id_col=0):
        ...
        if self.n_first is not None:
            ...
            corrmat = df.corr()
            columns = corrmat.nlargest(self.n_first, predict_column)
            columns = columns[predict_column].index
            corrmat = corrmat.nlargest(self.n_first, predict_column)[columns]
            correlations = corrmat[predict_column].abs()
...
#                     }
    ...
    def fit(self, estimator, test_ratio=0.1):
        ...
        self.best_score = 0
        self.estimator_results = pd.DataFrame(index=range(1),
                                              columns=['name', 'score'])
        estimator_name = estimator.__class__.__name__
        mean_outcome, std_outcome = self.run_kfold(estimator, estimator_name)
    ...
    def fit_all(self):
        ...
        idx = 0
        self.best_score = 0
        self.estimator_results = pd.DataFrame(index=range(len(self.estimators)),
                                              columns=['name', 'score'])
        for estimator in self.estimators:
            self.run_kfold(estimator, estimator.__class__.__name__, idx)
            idx += 1
        print('Résultats des estimateurs' + "\n")
        self.estimator_results.sort_values(by='score', ascending=False,
                                           inplace=True)
        print(self.estimator_results, "\n")
        if len(self.estimator_results) > 1:
            plt.figure()
            sns.barplot(x='score', y='name', ata=self.estimator_results,
                        palette="Set1")
            plt.show()
    ...
    def run_kfold(self, estimator, name, idx=0):
        kf = KFold(n_splits=15, shuffle=True,
                   random_state=np.random.randint(0, 255))
        outcomes = []
        fold = 0
...
#            y_train, y_test = self.Y[train_index], self.Y[test_index]
        ...
        print("\n", name, " - Mean Accuracy: {0}".format(mean_outcome),
              ' +/- ', std_outcome)
        print("\n" + '------------------------------------------------' + "\n")
    ...
    def fit_best(self, test_ratio=0.1):
        ...
        # Run the grid search
        grid_obj = GridSearchCV(self.best_estimator,
                                self.estimator_params[self.best_estimator_name],
                                scoring=acc_scorer)
    ...
    def best_feats(self):
        ...
        if hasattr(self.best_estimator, 'feature_importances_') and hasattr(self.best_estimator, 'estimators_'):
            ...
            plt.xticks(range(self.n_dimensions), X_train_columns[indices],
                       rotation=45)
        ...
        if hasattr(self.best_estimator, 'coef_'):
            ...
            plt.xticks(range(self.n_dimensions),
                       X_train_columns[indices[0][::-1]], rotation=45)
            plt.xlim([-1, self.n_dimensions])
            plt.show()
    ...
    def dump_result(self, X_predict, file_name='dump.csv'):
        ...
        output = pd.DataFrame({self.id_col: self.Id_test,
                               self.predict_column: predictions})
